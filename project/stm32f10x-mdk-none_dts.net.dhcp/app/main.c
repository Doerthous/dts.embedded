...
static void set_ip(uint8_t *ip, size_t len)
{
	if (len == 4) {
		DTS_NET_IPv4_ADDR(&ip0.addr,ip[0],ip[1],ip[2],ip[3]);
	}
}
...
int main(void)
{
	...
	int c = 0;
	while (1) {
		if (dhcp_client_setup("0.0.0.0", (const char *)eth0.mac_address, 
			set_ip)) {
			dhcp_client_start();
			while (1) {
				isr_poll();
				dhcp_loop();
				if (dhcp_client_completed()) {
